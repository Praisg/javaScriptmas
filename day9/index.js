...
const suitableRecipes = recipes.filter(recipe => {
  // Check if recipe has at least one ingredient Alice loves
  const hasLovedIngredient = recipe.ingredients.some(ingredient => 
    guest.loves.includes(ingredient)
  );
  
  // Check if recipe has no ingredients Alice dislikes
  const hasNoDislikedIngredients = !recipe.ingredients.some(ingredient => 
    guest.dislikes.includes(ingredient)
  );
  
  // Recipe is suitable if both conditions are met
  return hasLovedIngredient && hasNoDislikedIngredients;
});
